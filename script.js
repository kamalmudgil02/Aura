...
document.addEventListener('DOMContentLoaded', () => {
    ...
    function createCardHTML(product) {
        ...
        <div class="product-card">
            <a href="product.html#${product.id}" style="text-decoration:none;color:inherit;">
    ...
    function initHeroSlider(slides) {
        ...
        track.innerHTML = slides.map(s => {
            ...
            return `<div class="hero-slide" style="background-image:url('${img}');"><div class="hero-slide-content"><span class="badge-new">Just Dropped</span><h2>${s.title}</h2><p>$${s.price.toFixed(2)}</p><a href="product.html#${s.id}" class="btn btn-large">Shop Now</a></div></div>`;
        }).join('');
    ...
    if (pdpTitle && window.Store) {
        ...
        if (product) {
            ...
            if (categoryLink) { categoryLink.href = `collection.html#${product.category}`; categoryLink.textContent = product.category.charAt(0).toUpperCase() + product.category.slice(1); }
            ...
            if (cc) cc.innerHTML = '<div class="col-span-full p-6 text-center text-muted"><p class="mb-4">The product you\'re looking for doesn\'t exist.</p><a href="index.html" class="btn btn-primary">Return Home</a></div>';
        }
    }
    ...
    function renderUpsells() {
        ...
        upsellItems.innerHTML = picks.map(p => {
            ...
            return `<div style="min-width:110px;text-align:center;flex-shrink:0;"><a href="product.html#${p.id}"><img src="${img}" style="width:100px;height:100px;object-fit:cover;border-radius:var(--radius-sm);"></a><p class="text-xs fw-medium mt-1">${p.title}</p><p class="text-xs text-muted">$${p.price.toFixed(2)}</p></div>`;
        }).join('');
    }
